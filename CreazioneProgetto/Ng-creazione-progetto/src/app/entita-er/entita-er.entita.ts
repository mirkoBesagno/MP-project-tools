...
export class EntitaER implements IEntitaER {
  ...
  GetPerDiagrammaER() {
    debugger;
    let ritorno: string = "";
    let ritornoSec: string ="";
    let accetto=false;
    if (this.nomeEntita != "") {
      ritornoSec = " <br> class " + this.nomeEntita + " { ";
    }
    for (let index = 0; index < this.listaAttributi.listaAttributoEntita.length; index++) {
      ...
      switch (TipologiaAttributo[element.tipologia]) {
        ...
        case "fk":
          ...
          ritornoSec = ritornoSec + "<br> " + element.tipoAttributo + " " + element.nomeAttributo+ ' ';
          accetto = true;
          break;
        case "vettore":
        case "vett":
    ...
    if (this.nomeEntita != "" && accetto == true) {
      ritornoSec = ritornoSec + "<br> } "
    }
    else{
      ritornoSec = "";
    }
    ritorno = ritorno + ritornoSec;
    return ritorno;
  }
